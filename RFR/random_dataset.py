...
# A位元素的列名称，从第3列到第15列
columns_for_Aelements = list(df.columns[2:15])
# B位元素的列名称，从第16列到第19列
columns_for_Belements = list(df.columns[15:19])
# 所有的列
columns_for_all = list(df.columns[:20])
...
def generate_random_numbers_with_features():
    ...
    feature_values[df.columns[19]] = round(WE * 0.1, 2)         # WE
