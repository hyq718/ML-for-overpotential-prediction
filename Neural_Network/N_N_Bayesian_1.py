...
class BayesianNeuralNetwork:
    ...
    def _initialize_bayesian_parameters(self):
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            std = np.sqrt(1.0 / (in_dim + out_dim))
    ...
    def _backward(self, x, y, weights, biases):
        ...
        for l in range(n_layers - 1, -1, -1):
            ...
            grad_log_var_weights[l] = 0.5 * epsilon_w * np.dot(a_prev.T, delta)  # (in, out)
            ...
            grad_log_var_biases[l] = 0.5 * epsilon_b * np.sum(delta, axis=0)     # (out,)
    ...
    def _custom_loss(self, y_true, y_pred):
        error = y_pred - y_true
        relative_error = error / (y_true + 1e-8)
        
        # 1. 误差在±10%内：MSE
        mask = np.abs(relative_error) <= 0.1
        loss_within = np.mean((y_pred[mask] - y_true[mask])**2)
        
        # 2. 误差超±10%：指数惩罚
        loss_outside = np.mean(10 * np.exp(3 * np.abs(relative_error[~mask])))
        
        return loss_within + loss_outside
    ...
    def train(self, X, y, verbose=True):
        ...
        for epoch in range(self.epochs):
            ...
            # 使用自定义损失
            custom_loss_value = self._custom_loss(y, preds)  # 修改后的损失
            kl_loss = self._compute_kl_divergence()
            total_loss = custom_loss_value + self.kl_weight * kl_loss
            ...
            if verbose and epoch % 100 == 0:
                print(f"Epoch {epoch}, Custom Loss: {custom_loss_value:.4f}, KL: {kl_loss:.4f}, Total Loss: {total_loss:.4f}")
...
def cross_validate(model_class, X, y, k=5, **model_kwargs):
    ...
    for train_idx, test_idx in kf.split(X):
        ...
        #print("NaN in y_true:", np.isnan(y[test_idx]).any())
        #print(y[test_idx])
...
def plot_prediction_with_confidence_interval(model, X, y_true, n_samples=50):
    ...
    # 确保 y_pred 和 y_std 是一维数组
    y_pred = np.squeeze(y_pred)  # 从 (n_samples, 1) 变为 (n_samples,)
    y_std = np.squeeze(y_std)    # 同上
